function [w_cart,dx,lonlat_ref,x,y,w_lonlat,lon,lat] = ...
    read_glob_current_w_v2_slightFiltered(day,lonlat_choose,dx,model)
% Load v2 geostrophic velocity at a specified day of the year 2011
...
folder_data = [ pwd '/data/globcurrent/' ...
    'v2_0_global_025_deg_geostrophic_2011/'];
if day<=201 % The first and second part of the year are in differnt folders
    folder_data = [folder_data day_str '/' ];
else
    folder_data = [folder_data 'next_days/' day_str '_' ];  
end
folder_data = [ folder_data ...
    date_str '-GLOBCURRENT-L4-CURgeo_0m-ALT_OI-v02.0-fv01.0.nc'];
...
% Change the origin of the spherical coordinates
if any(lonlat_choose(1:2)<0)
    lonlat_choose(1:2)=lonlat_choose(1:2)+360;
end
...
% standard deviation of the Gaussian filter ( ~ cutoff frequency)
sigma_filter = 5e3;
model.advection.sigma_filter = sigma_filter;

% Gaussian filter
filter_perso = design_filter_5(model);

% Filtering by convolution
w_cart(:,:,1) = conv2(w_cart(:,:,1),filter_perso,'same');
w_cart(:,:,2) = conv2(w_cart(:,:,2),filter_perso,'same');

% Plot
if bool_plot
    figure;imagesc(x,y,sqrt(sum(w_cart.^2,3))');
    axis xy; axis equal;colorbar;
    title('Slighltly filtered cartesian KE field')
end
