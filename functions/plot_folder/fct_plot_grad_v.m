function fct_plot_grad_v(model,w)
% Plot the norm of the gradient of velocity
%
...
folder_simu = [ folder_simu '/n_grad_w'];
map = 'default';
...
if isfield(model.grid,'lonlat')
    ...
    n_grad_w = fct_cart2sph(x,y,n_grad_w,LON,LAT,lonlat_ref);
    x = lon;y=lat;
    ...
    %     type_data = model.type_data;
    folder_simu = model.folder.folder_simu;
    folder_simu = [ folder_simu '/n_grad_w_lonlat'];
    map = 'default';
    loc_colorbar = 'southoutside';
    ax = [x(end)-x(1) y(end)-y(1)] ;
    aspect_ratio = ax(2)/ax(1);
